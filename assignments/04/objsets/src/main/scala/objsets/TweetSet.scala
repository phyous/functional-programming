...
abstract class TweetSet {
  ...
  def descendingByRetweet: TweetList = {
    val topTweet = try {
      Some(this.mostRetweeted)
    } catch {
      case e: java.util.NoSuchElementException => None
    }
    topTweet match {
      case None => Nil
      case Some(value) => new Cons(value, this.remove(value).descendingByRetweet)
    }
  }
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
    ...
    val leftResult = left.filterAcc(p, newAcc)
    right.filterAcc(p, leftResult)
  }
...
object GoogleVsApple {
  ...
  lazy val googleTweets: TweetSet = tweets.filter(t => google.exists(str => t.text.contains(str)))
  lazy val appleTweets: TweetSet = tweets.filter(t => apple.exists(str => t.text.contains(str)))
